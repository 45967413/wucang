...
jQuery(function () {
  ...
  $('.banner').on('init', function (event, slick, currentSlide, nextSlide) {
    ...
    TweenLite.to($(".banner-item").eq(0).find('figure img'), 0, {
      scale: 1.15
    });
    TweenMax.to($(".banner-item").eq(0).find('figure img'), 5.4, {
      scale: 1,
  ...
  $(".banner").slick({
    ...
    TweenLite.to($(".banner-item").eq(nextSlide).find('figure img'), 0, {
      scale: 1.15
    });
    TweenMax.to($(".banner-item").eq(nextSlide).find('figure img'), 5.4, {
      scale: 1,
      ease: Power0.easeNone
    });
  });
